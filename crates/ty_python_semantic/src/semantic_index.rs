...
use crate::semantic_index::lvalue::{
    FileScopeId, LValueTable, NodeWithScopeKey, NodeWithScopeRef, Scope, ScopeId, ScopeKind,
    ScopedLValueId,
};
...
type LValueSet = hashbrown::HashMap<ScopedLValueId, (), FxBuildHasher>;
...
/// Returns the lvalue table for a specific `scope`.
///
/// Using [`lvalue_table`] over [`semantic_index`] has the advantage that
/// Salsa can avoid invalidating dependent queries if this scope's lvalue table
/// is unchanged.
#[salsa::tracked(returns(deref))]
pub(crate) fn lvalue_table<'db>(db: &'db dyn Db, scope: ScopeId<'db>) -> Arc<LValueTable> {
    let file = scope.file(db);
    let _span = tracing::trace_span!("lvalue_table", scope=?scope.as_id(), ?file).entered();
    let index = semantic_index(db, file);

    index.lvalue_table(scope.file_scope_id(db))
}
...
pub(crate) fn attribute_assignments<'db, 's>(
    ...
    ChildrenIter::new(index, class_scope_id).filter_map(|(child_scope_id, scope)| {
        ...
        let lvalue_table = index.lvalue_table(function_scope_id);
        let lvalue = lvalue_table.lvalue_id_by_instance_attribute_name(name)?;
        let use_def = &index.use_def_maps[function_scope_id];
        Some((use_def.public_bindings(lvalue), function_scope_id))
    })
}
...
pub(crate) struct SemanticIndex<'db> {
    /// List of all lvalue tables in this file, indexed by scope.
    lvalue_tables: IndexVec<FileScopeId, Arc<LValueTable>>,
    ...
    globals_by_scope: FxHashMap<FileScopeId, FxHashSet<ScopedLValueId>>,
...
impl<'db> SemanticIndex<'db> {
    /// Returns the lvalue table for a specific scope.
    ///
    /// Use the Salsa cached [`lvalue_table()`] query if you only need the
    /// lvalue table for a single scope.
    #[track_caller]
    pub(super) fn lvalue_table(&self, scope_id: FileScopeId) -> Arc<LValueTable> {
        self.lvalue_tables[scope_id].clone()
    }
    ...
    pub(crate) fn eager_snapshot(
        ...
        let Some(symbol_id) = self.lvalue_tables[enclosing_scope].lvalue_id_by_name(symbol) else {
            return EagerSnapshotResult::NotFound;
        };
        let key = EagerSnapshotKey {
            enclosing_scope,
            enclosing_lvalue: symbol_id,
            nested_scope,
        };
...
mod tests {
    ...
    use crate::semantic_index::lvalue::{
        FileScopeId, LValueTable, Scope, ScopeKind, ScopedLValueId,
    };
    ...
    impl UseDefMap<'_> {
        fn first_public_binding(&self, symbol: ScopedLValueId) -> Option<Definition<'_>> {
    ...
    fn names(table: &LValueTable) -> Vec<String> {
        table
            .lvalues()
            .filter_map(|lvalue| Some(lvalue.as_name()?.to_string()))
            .collect()
    }
    ...
    fn empty() {
        ...
        let global_table = lvalue_table(&db, global_scope(&db, file));
    ...
    fn import() {
        ...
        let global_table = lvalue_table(&db, scope);

        assert_eq!(names(global_table), vec!["foo"]);
        let foo = global_table.lvalue_id_by_name("foo").unwrap();
    ...
    fn assign() {
        ...
        let use_def = use_def_map(&db, scope);
        let binding = use_def
            .first_public_binding(global_table.lvalue_id_by_name("x").expect("symbol exists"))
            .unwrap();
    ...
    fn augmented_assignment() {
        ...
        let use_def = use_def_map(&db, scope);
        let binding = use_def
            .first_public_binding(global_table.lvalue_id_by_name("x").unwrap())
            .unwrap();
...
",
        ...
        let class_table = index.lvalue_table(class_scope_id);
        assert_eq!(names(&class_table), vec!["x"]);

        let use_def = index.use_def_map(class_scope_id);
        let binding = use_def
            .first_public_binding(class_table.lvalue_id_by_name("x").expect("symbol exists"))
            .unwrap();
        assert!(matches!(binding.kind(&db), DefinitionKind::Assignment(_)));
    }
...
",
        ...
        let index = semantic_index(&db, file);
        let global_table = index.lvalue_table(FileScopeId::global());
        ...
        let function_table = index.lvalue_table(function_scope_id);
...
",
    ...
    fn lambda_parameter_symbols() {
        ...
        let lambda_table = index.lvalue_table(lambda_scope_id);
        assert_eq!(
            names(&lambda_table),
            vec!["a", "b", "c", "d", "args", "kwargs"],
        );
        ...
        for name in ["a", "b", "c", "d"] {
            let binding = use_def
                .first_public_binding(lambda_table.lvalue_id_by_name(name).expect("symbol exists"))
                .unwrap();
            assert!(matches!(binding.kind(&db), DefinitionKind::Parameter(_)));
        }
        let args_binding = use_def
...
",
        ...
        let comprehension_symbol_table = index.lvalue_table(comprehension_scope_id);
...
",
        ...
        let inner_comprehension_symbol_table = index.lvalue_table(inner_comprehension_scope_id);
...
",
        ...
        for name in ["x", "y"] {
            let binding = use_def
                .first_public_binding(global_table.lvalue_id_by_name(name).expect("symbol exists"))
...
",
        ...
        let func1_table = index.lvalue_table(func_scope1_id);
        let func2_table = index.lvalue_table(func_scope2_id);
...
",
        ...
        let ann_table = index.lvalue_table(ann_scope_id);
        ...
        let func_table = index.lvalue_table(func_scope_id);
...
",
        ...
        assert_eq!(class_scope.kind(), ScopeKind::Class);
        assert_eq!(class_scope_id.to_scope_id(&db, file).name(&db), "C");
        assert_eq!(names(&index.lvalue_table(class_scope_id)), vec!["x"]);
    }
...
",
        ...
        let global_table = lvalue_table(&db, global_scope_id);

        assert!(global_table.lvalue_by_name("Foo").unwrap().is_used());
        assert_eq!(
            names(global_table),
...
",
    ...
    fn for_loops_simple_unpacking() {
        ...
        let y_binding = use_def
            .first_public_binding(global_table.lvalue_id_by_name("y").unwrap())
            .unwrap();
    ...
    fn for_loops_complex_unpacking() {
        ...
        let use_def = use_def_map(&db, scope);
        let binding = use_def
            .first_public_binding(global_table.lvalue_id_by_name("a").unwrap())
            .unwrap();
