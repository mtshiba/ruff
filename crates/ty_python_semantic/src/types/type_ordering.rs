...
/// Return an [`Ordering`] based on the structural content of two types, without requiring
/// normalization. This is used to sort elements in recursively-defined unions where
/// normalization may not yet have been applied, and where salsa ID-based ordering
/// may be non-deterministic due to non-deterministic query execution order.
pub(super) fn structural_type_ordering<'db>(
    ...
    match (left, right) {
        ...
        // For types identified by salsa IDs, compare by name to ensure stable ordering.
        ...
        (Type::ModuleLiteral(left), Type::ModuleLiteral(right)) => {
            left.module(db).name(db).cmp(right.module(db).name(db))
        }
        (Type::ModuleLiteral(_), _) => Ordering::Less,
        (_, Type::ModuleLiteral(_)) => Ordering::Greater,
        ...
        (Type::GenericAlias(left), Type::GenericAlias(right)) => {
            left.origin(db).name(db).cmp(right.origin(db).name(db))
        }
        (Type::GenericAlias(_), _) => Ordering::Less,
        (_, Type::GenericAlias(_)) => Ordering::Greater,
        ...
        (Type::TypeIs(left), Type::TypeIs(right)) => {
            structural_guard_like_ordering(db, *left, *right)
        }
        (Type::TypeIs(_), _) => Ordering::Less,
        (_, Type::TypeIs(_)) => Ordering::Greater,
        ...
        (Type::NominalInstance(left), Type::NominalInstance(right)) => {
            left.class_name(db).cmp(right.class_name(db))
        }
        (Type::NominalInstance(_), _) => Ordering::Less,
        (_, Type::NominalInstance(_)) => Ordering::Greater,
        ...
        (Type::TypeVar(left), Type::TypeVar(right)) => left.name(db).cmp(right.name(db)),
        (Type::TypeVar(_), _) => Ordering::Less,
        (_, Type::TypeVar(_)) => Ordering::Greater,
        ...
        (Type::Union(left), Type::Union(right)) => {
            ...
            left_elements
                ...
                .then_with(|| {
                    for (l, r) in left_elements.iter().zip(right_elements.iter()) {
                        let ord = structural_type_ordering(db, l, r);
        ...
        (Type::Intersection(left), Type::Intersection(right)) => {
            ...
            left_positive
                .len()
                .cmp(&right_positive.len())
                .then_with(|| {
                    for (l, r) in left_positive.iter().zip(right_positive.iter()) {
                        let ord = structural_type_ordering(db, l, r);
                        if ord != Ordering::Equal {
                            return ord;
                        }
                    ...
                    left_negative
                        .len()
                        .cmp(&right_negative.len())
                        .then_with(|| {
                            for (l, r) in left_negative.iter().zip(right_negative.iter()) {
                                let ord = structural_type_ordering(db, l, r);
                                if ord != Ordering::Equal {
                                    return ord;
                                }
...
/// Structural variant of [`guard_like_ordering`] that uses [`structural_type_ordering`].
fn structural_guard_like_ordering<'db, T: TypeGuardLike<'db>>(
    db: &'db dyn Db,
    left: T,
    right: T,
) -> Ordering {
    ...
    match (left.place_info(db), right.place_info(db)) {
        ...
        (None, None) => structural_type_ordering(db, &left_ty, &right_ty),

        (Some(_), Some(_)) => match left.place_name(db).cmp(&right.place_name(db)) {
            Ordering::Equal => structural_type_ordering(db, &left_ty, &right_ty),
            ordering => ordering,
        },
    }
}
