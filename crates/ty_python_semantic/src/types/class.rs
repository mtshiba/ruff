...
use crate::types::{
    ...
    DeprecatedInstance, DivergenceKind, DivergentType, FindLegacyTypeVarsVisitor,
    ...
    determine_upper_bound, infer_expression_type, infer_unpack_types,
};
use crate::{
                ...
                if let Some(all_qualifiers) = annotation.is_bare_final() {
                    if let Some(value) = assignment.value(&module) {
                        ...
                        return Member::inferred(
                            Place::bound(inferred_ty.recursive_type_normalized(db, &visitor))
