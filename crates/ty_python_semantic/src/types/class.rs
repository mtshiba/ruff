...
use crate::{
    ...
    semantic_index::{
        ...
        lvalue::ScopeId,
        lvalue_table, semantic_index, use_def_map,
    },
    symbol::{
...
impl<'db> ClassLiteral<'db> {
    ...
    fn own_fields(self, db: &'db dyn Db) -> FxOrderMap<Name, (Type<'db>, Option<Type<'db>>)> {
        ...
        let table = lvalue_table(db, class_body_scope);

        let use_def = use_def_map(db, class_body_scope);
        for (lvalue_id, declarations) in use_def.all_public_declarations() {
            ...
            let symbol = table.lvalue(lvalue_id);
    ...
    fn implicit_instance_attribute(
        ...
        let class_table = lvalue_table(db, class_body_scope);
            ...
            attribute_assignments(db, class_body_scope, name)
        {
            ...
            let is_method_visible = if let Some(method_def) = method_scope.node(db).as_function() {
                ...
                let method_symbol = class_table.lvalue_id_by_name(&method_def.name).unwrap();
                class_map
    ...
    fn own_instance_member(self, db: &'db dyn Db, name: &str) -> SymbolAndQualifiers<'db> {
        ...
        let body_scope = self.body_scope(db);
        let table = lvalue_table(db, body_scope);

        if let Some(lvalue_id) = table.lvalue_id_by_name(name) {
