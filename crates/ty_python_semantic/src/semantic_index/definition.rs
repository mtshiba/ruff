...
use crate::semantic_index::lvalue::{FileScopeId, ScopeId, ScopedLValueId};
...
pub struct Definition<'db> {
    ...
    /// The lvalue defined.
    pub(crate) lvalue: ScopedLValueId,
...
pub(crate) struct StarImportDefinitionNodeRef<'a> {
    ...
    pub(crate) lvalue_id: ScopedLValueId,
}
...
impl<'db> DefinitionNodeRef<'db> {
    ...
    pub(super) unsafe fn into_owned(self, parsed: ParsedModule) -> DefinitionKind<'db> {
        match self {
            ...
            DefinitionNodeRef::ImportStar(star_import) => {
                let StarImportDefinitionNodeRef { node, lvalue_id } = star_import;
    ...
    pub(super) fn key(self) -> DefinitionNodeKey {
        match self {
            ...
            Self::ImportStar(StarImportDefinitionNodeRef { node, lvalue_id: _ }) => node
                .names
                .iter()
...
impl StarImportDefinitionKind {
    ...
    pub(crate) fn lvalue_id(&self) -> ScopedLValueId {
        self.lvalue_id
    }
}
