...
use crate::semantic_index::lvalue::{
    FileScopeId, LValue, LValueTableBuilder, NodeWithScopeKey, NodeWithScopeKind, NodeWithScopeRef,
    Scope, ScopeId, ScopeKind, ScopedLValueId,
};
...
pub(super) struct SemanticIndexBuilder<'db> {
    ...
    lvalue_tables: IndexVec<FileScopeId, LValueTableBuilder>,
...
impl<'db> SemanticIndexBuilder<'db> {
    ...
    fn push_scope_with_parent(
        ...
        self.lvalue_tables.push(LValueTableBuilder::default());
        self.use_def_maps
    ...
    fn pop_scope(&mut self) -> FileScopeId {
        ...
        for enclosing_scope_info in self.scope_stack.iter().rev() {
            ...
            let enclosing_lvalue_table = &self.lvalue_tables[enclosing_scope_id];

            for nested_lvalue in self.lvalue_tables[popped_scope_id].lvalues() {
                // Skip this lvalue if this enclosing scope doesn't contain any bindings for it.
                // Note that even if this lvalue is bound in the popped scope,
                // it may refer to the enclosing scope bindings
                // so we also need to snapshot the bindings of the enclosing scope.

                let Some(enclosing_lvalue_id) =
                    enclosing_lvalue_table.lvalue_id_by_lvalue(nested_lvalue)
                else {
                    continue;
                };
                let enclosing_lvalue = enclosing_lvalue_table.lvalue(enclosing_lvalue_id);

                // Snapshot the state of this lvalue that are visible at this point in this
                // enclosing scope.
                let key = EagerSnapshotKey {
                    enclosing_scope: enclosing_scope_id,
                    enclosing_lvalue: enclosing_lvalue_id,
            ...
            // TODO: If the lvalue has never been rewritten, they are applicable.
    ...
    fn current_lvalue_table(&mut self) -> &mut LValueTableBuilder {
        let scope_id = self.current_scope();
        &mut self.lvalue_tables[scope_id]
    }
    ...
    /// Add a symbol to the lvalue table and the use-def map.
    /// Return the [`ScopedLValueId`] that uniquely identifies the symbol in both.
    fn add_symbol(&mut self, name: Name) -> ScopedLValueId {
        let (lvalue_id, added) = self.current_lvalue_table().add_symbol(name);
        if added {
            self.current_use_def_map_mut().add_lvalue(lvalue_id);
        }
        lvalue_id
    }

    fn add_lvalue(&mut self, lvalue: LValue) -> ScopedLValueId {
        let (lvalue_id, added) = self.current_lvalue_table().add_lvalue(lvalue);
        if added {
            self.current_use_def_map_mut().add_lvalue(lvalue_id);
        }
        lvalue_id
    ...
    fn mark_lvalue_bound(&mut self, id: ScopedLValueId) {
        self.current_lvalue_table().mark_lvalue_bound(id);
    }

    fn mark_lvalue_declared(&mut self, id: ScopedLValueId) {
        self.current_lvalue_table().mark_lvalue_declared(id);
    }

    fn mark_lvalue_used(&mut self, id: ScopedLValueId) {
        self.current_lvalue_table().mark_lvalue_used(id);
    }
    ...
    fn add_definition(
        ...
        let (definition, num_definitions) =
            self.push_additional_definition(lvalue, definition_node);
        debug_assert_eq!(
            num_definitions,
            1,
    ...
    fn push_additional_definition(
        ...
        if category.is_binding() {
            self.mark_lvalue_bound(lvalue);
        }
        if category.is_declaration() {
            self.mark_lvalue_declared(lvalue);
        }

        let use_def = self.current_use_def_map_mut();
        match category {
            DefinitionCategory::DeclarationAndBinding => {
                use_def.record_declaration_and_binding(lvalue, definition);
            }
            DefinitionCategory::Declaration => use_def.record_declaration(lvalue, definition),
            DefinitionCategory::Binding => use_def.record_binding(lvalue, definition),
        }
    ...
    fn add_definition_by_definition_kind(
        ...
        self.current_use_def_map_mut()
            .record_binding(lvalue, definition);
        definition
    }
    ...
    fn register_attribute_assignment(
        ...
        if self.is_method_of_class().is_some() {
            ...
            if accessed_object_refers_to_first_parameter {
                let mut lvalue = LValue::name(object.as_name_expr().unwrap().id.clone())
                    .member(attr.id().clone());
                lvalue.mark_instance_attribute();
                let id = self.add_lvalue(lvalue);
    ...
    fn with_type_params(
        ...
        if let Some(type_params) = type_params {
            ...
            for type_param in &type_params.type_params {
                ...
                self.mark_lvalue_bound(symbol);
                self.mark_lvalue_declared(symbol);
    ...
    pub(super) fn build(mut self) -> SemanticIndex<'db> {
        ...
        let mut lvalue_tables: IndexVec<_, _> = self
            .lvalue_tables
            .into_iter()
        ...
        match stmt {
                ...
                for (alias_index, alias) in node.names.iter().enumerate() {
                    if &alias.name == "*" {
                        ...
                        for export in exported_names(self.db, referenced_module) {
                            ...
                            let pre_definition =
                                self.current_use_def_map().single_lvalue_snapshot(symbol_id);
            ...
            ast::Stmt::Global(ast::StmtGlobal { range: _, names }) => {
                for name in names {
                    ...
                    let symbol_table = self.current_lvalue_table();
                    let symbol = symbol_table.lvalue(symbol_id);
            ...
            ast::Stmt::Delete(ast::StmtDelete { targets, range: _ }) => {
                for target in targets {
                    if let ast::Expr::Name(ast::ExprName { id, .. }) = target {
                        ...
                        self.current_lvalue_table().mark_lvalue_used(symbol_id);
                    }
                }
                walk_stmt(self, stmt);
            }
