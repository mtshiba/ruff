...
//! `public_bindings` and `public_definitions` vectors indexed by [`ScopedLValueId`]. The values in
...
//! each lvalue, the builder tracks the `LValueState` (`Bindings` and `Declarations`)
...
//! the new symbol (before we process the first binding), we create a new undefined `LValueState`
...
use self::symbol_state::{
    Bindings, Declarations, EagerSnapshot, LValueState, LiveBindingsIterator, LiveDeclaration,
    LiveDeclarationsIterator, ScopedDefinitionId,
};
use crate::node_key::NodeKey;
use crate::semantic_index::ast_ids::ScopedUseId;
use crate::semantic_index::definition::Definition;
use crate::semantic_index::lvalue::{FileScopeId, ScopeKind, ScopedLValueId};
...
pub(crate) struct UseDefMap<'db> {
    ...
    /// [`LValueState`] visible at end of scope for each lvalue.
    public_lvalues: IndexVec<ScopedLValueId, LValueState>,
...
impl<'db> UseDefMap<'db> {
    ...
    pub(crate) fn public_bindings(
        ...
        self.bindings_iterator(self.public_lvalues[lvalue].bindings())
    }
    ...
    pub(crate) fn public_declarations<'map>(
        ...
        let declarations = self.public_lvalues[lvalue].declarations();
        self.declarations_iterator(declarations)
    }

    pub(crate) fn all_public_declarations<'map>(
        &'map self,
    ) -> impl Iterator<Item = (ScopedLValueId, DeclarationsIterator<'map, 'db>)> + 'map {
        (0..self.public_lvalues.len())
            .map(ScopedLValueId::from_usize)
            .map(|lvalue_id| (lvalue_id, self.public_declarations(lvalue_id)))
    }

    pub(crate) fn all_public_bindings<'map>(
        &'map self,
    ) -> impl Iterator<Item = (ScopedLValueId, BindingWithConstraintsIterator<'map, 'db>)> + 'map
    {
        (0..self.public_lvalues.len())
            .map(ScopedLValueId::from_usize)
            .map(|lvalue_id| (lvalue_id, self.public_bindings(lvalue_id)))
    }
...
/// Uniquely identifies a snapshot of a lvalue state that can be used to resolve a reference in a
/// nested eager scope.
///
...
pub(crate) struct EagerSnapshotKey {
    ...
    /// The referenced lvalue (in the enclosing scope)
    pub(crate) enclosing_lvalue: ScopedLValueId,
    /// The nested eager scope containing the reference
    pub(crate) nested_scope: FileScopeId,
}

/// A snapshot of lvalue states that can be used to resolve a reference in a nested eager scope.
...
pub(super) struct FlowSnapshot {
    lvalue_states: IndexVec<ScopedLValueId, LValueState>,
    scope_start_visibility: ScopedVisibilityConstraintId,
    reachability: ScopedVisibilityConstraintId,
}
...
pub(super) struct UseDefMapBuilder<'db> {
    ...
    /// Currently live bindings and declarations for each lvalue.
    lvalue_states: IndexVec<ScopedLValueId, LValueState>,

    /// Snapshots of lvalue states in this scope that can be used to resolve a reference in a
    /// nested eager scope.
    eager_snapshots: EagerSnapshots,
...
impl<'db> UseDefMapBuilder<'db> {
    ...
    pub(super) fn add_lvalue(&mut self, lvalue: ScopedLValueId) {
        let new_lvalue = self
            .lvalue_states
            .push(LValueState::undefined(self.scope_start_visibility));
        debug_assert_eq!(lvalue, new_lvalue);
    }

    pub(super) fn record_binding(&mut self, symbol: ScopedLValueId, binding: Definition<'db>) {
        let def_id = self.all_definitions.push(Some(binding));
        let lvalue_state = &mut self.lvalue_states[symbol];
        self.declarations_by_binding
            .insert(binding, lvalue_state.declarations().clone());
        lvalue_state.record_binding(def_id, self.scope_start_visibility, self.is_class_scope);
    }
    ...
    pub(super) fn record_narrowing_constraint(&mut self, predicate: ScopedPredicateId) {
        ...
        for state in &mut self.lvalue_states {
            state
    ...
    /// Snapshot the state of a single lvalue at the current point in control flow.
    ///
    ...
    pub(super) fn single_lvalue_snapshot(&self, lvalue: ScopedLValueId) -> LValueState {
        self.lvalue_states[lvalue].clone()
    }
    ...
    ///   the visibility constraints is only important for lvalues that did not have any new
    ///   definitions inside either the "if-predicate-true" branch or the "if-predicate-false" branch.
    ///
    /// - We only snapshot the state for a single lvalue prior to the definition, rather than doing
    ...
    pub(super) fn record_and_negate_star_import_visibility_constraint(
        ...
        symbol: ScopedLValueId,
        pre_definition_state: LValueState,
    ) {
    ...
    pub(super) fn simplify_visibility_constraints(&mut self, snapshot: FlowSnapshot) {
        debug_assert!(self.lvalue_states.len() >= snapshot.lvalue_states.len());
        ...
        // Note that this loop terminates when we reach a lvalue not present in the snapshot.
        // This means we keep visibility constraints for all new lvalues, which is intended,
        // since these lvalues have been introduced in the corresponding branch, which might
        // be subject to visibility constraints. We only simplify/reset visibility constraints
        // for lvalues that have the same bindings and declarations present compared to the
        // snapshot.
        for (current, snapshot) in self.lvalue_states.iter_mut().zip(snapshot.lvalue_states) {
    ...
    pub(super) fn record_declaration(
        ...
        let lvalue_state = &mut self.lvalue_states[lvalue];
        self.bindings_by_declaration
            .insert(declaration, lvalue_state.bindings().clone());
        lvalue_state.record_declaration(def_id);
    }
    ...
    pub(super) fn record_use(
        ...
        // We have a use of a lvalue; clone the current bindings for that lvalue, and record them
        // as the live bindings for this use.
        let new_use = self
            .bindings_by_use
            .push(self.lvalue_states[lvalue].bindings().clone());
        debug_assert_eq!(use_id, new_use);

        // Track reachability of all uses of lvalues to silence `unresolved-reference`
    ...
    pub(super) fn snapshot_eager_state(
        &mut self,
        enclosing_lvalue: ScopedLValueId,
        scope: ScopeKind,
        is_bound: bool,
        ...
        if scope.is_class() || !is_bound {
            self.eager_snapshots.push(EagerSnapshot::Constraint(
                self.lvalue_states[enclosing_lvalue]
                    .bindings()
                    .unbound_narrowing_constraint(),
            ))
        } else {
            self.eager_snapshots.push(EagerSnapshot::Bindings(
                self.lvalue_states[enclosing_lvalue].bindings().clone(),
            ))
        }
    }

    /// Take a snapshot of the current visible-lvalues state.
    pub(super) fn snapshot(&self) -> FlowSnapshot {
        FlowSnapshot {
            lvalue_states: self.lvalue_states.clone(),
            scope_start_visibility: self.scope_start_visibility,
            reachability: self.reachability,
        }
    }

    /// Restore the current builder lvalues state to the given snapshot.
    pub(super) fn restore(&mut self, snapshot: FlowSnapshot) {
        // We never remove lvalues from `lvalue_states` (it's an IndexVec, and the lvalue
        // IDs must line up), so the current number of known lvalues must always be equal to or
        // greater than the number of known lvalues in a previously-taken snapshot.
        let num_lvalues = self.lvalue_states.len();
        debug_assert!(num_lvalues >= snapshot.lvalue_states.len());

        // Restore the current visible-definitions state to the given snapshot.
        self.lvalue_states = snapshot.lvalue_states;
        self.scope_start_visibility = snapshot.scope_start_visibility;
        self.reachability = snapshot.reachability;

        // If the snapshot we are restoring is missing some lvalues we've recorded since, we need
        // to fill them in so the lvalue IDs continue to line up. Since they don't exist in the
        // snapshot, the correct state to fill them in with is "undefined".
        self.lvalue_states.resize(
            num_lvalues,
            LValueState::undefined(self.scope_start_visibility),
        );
    }

    /// Merge the given snapshot into the current state, reflecting that we might have taken either
    /// path to get here. The new state for each lvalue should include definitions from both the
    ...
    pub(super) fn merge(&mut self, snapshot: FlowSnapshot) {
        ...
        let mut snapshot_definitions_iter = snapshot.lvalue_states.into_iter();
        for current in &mut self.lvalue_states {
            if let Some(snapshot) = snapshot_definitions_iter.next() {
                ...
                current.merge(
                    LValueState::undefined(snapshot.scope_start_visibility),
                    &mut self.narrowing_constraints,
                    &mut self.visibility_constraints,
                );
    ...
    pub(super) fn finish(mut self) -> UseDefMap<'db> {
        ...
        self.lvalue_states.shrink_to_fit();
        ...
        UseDefMap {
            ...
            public_lvalues: self.lvalue_states,
